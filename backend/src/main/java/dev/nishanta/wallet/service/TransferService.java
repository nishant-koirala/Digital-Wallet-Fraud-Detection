...
import dev.nishanta.wallet.domain.EntryType;
import dev.nishanta.wallet.domain.FraudFlag;
import dev.nishanta.wallet.domain.LedgerEntry;
import dev.nishanta.wallet.domain.Transaction;
import dev.nishanta.wallet.domain.Wallet;
import dev.nishanta.wallet.fraud.FraudRule;
import dev.nishanta.wallet.ledger.WalletBalanceCalculator;
import dev.nishanta.wallet.repository.FraudFlagRepository;
import dev.nishanta.wallet.repository.LedgerEntryRepository;
import dev.nishanta.wallet.repository.TransactionRepository;
...
public class TransferService {
    ...
    @Transactional
    public Transaction transfer(String idempotencyKey, UUID fromWalletId, UUID toWalletId, BigDecimal amount) {
        ...
        if (senderBalance.compareTo(amount) < 0) {
            throw new IllegalStateException("Insufficient balance in wallet " + fromWallet.getId());
        }
        ...
        if (!firedRules.isEmpty()) {
            ...
            // approves this transaction (that's the next piece we'll build).
            return transaction;
        }
